...
class MultiFileReader:
    ...
    global my_bucket_name

    def __init__(self):
        self._open_files = {}
        self.bucket_name = my_bucket_name

    def helper_open(self, f_name, work_type, file_path):
        client = storage.Client(f_name)
        bucket = client.bucket(self.bucket_name)
        blob = bucket.get_blob(file_path + f_name)
        return blob.open(work_type)

    def read(self, locs, n_bytes, file_path):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = self.helper_open(f_name, 'rb', file_path)
...
class InvertedIndex:
    ...
    def __init__(self, docs={}):
        ...
        self.tf_term_doc = {}
        self.N = 0
        self.DIR_NAME = ''  # "title_index/postings_gcp_title_index"
    ...
    def write_a_posting_list(b_w_pl, bucket_name):
        ...
        with closing(MultiFileWriter("../../IR_proj/true_final_indexes_from_gcp/ok now for real", bucket_id, bucket_name)) as writer:
